...
if __name__ == "__main__":
    ...
    for model_parameters in model_list:
        print("check this - ", model_parameters)
  	if (model_parameters[0] == 0):
	    ...
	    gp_model.fit(np.log(training_inputs),np.log(training_data))
        elif (args.predictor_transform==0 and args.response_transform==1):
	    gp_model.fit(training_inputs,np.log(training_data))
        elif (args.predictor_transform==1 and args.response_transform==0):
	    gp_model.fit(np.log(training_inputs),training_data)
        elif (args.predictor_transform==0 and args.response_transform==0):
	    gp_model.fit(training_inputs,training_data)

        joblib.dump(gp_model, "GP.joblib") 
        model_size = os.path.getsize('GP.joblib')
        print("GP model size: %f bytes"%(model_size))
	...
	for k in range(validation_set_size):
	    input_tuple = test_inputs[k,:]*1. if args.predictor_transform==0 else np.log(test_inputs[k,:]*1.) # Note: without this cast from into to float, interpolation produces zeros
            ...
            model_val = gp_model.predict([input_tuple])[0]
	...
	for k in range(validation_set_size):
	    ...
	    print("Validation Error is ",validation_error_metrics, " with time: ", timers)
    ...
    test_error_metrics[2] = np.average(np.asarray(prediction_errors[0])**2)
    test_error_metrics[3] = np.std(np.asarray(prediction_errors[0])**2,ddof=1)
    test_error_metrics[4] = scst.gmean(prediction_errors[1])
    test_error_metrics[5] = np.exp(np.std(np.log(prediction_errors[1]),ddof=1))
    test_error_metrics[6] = np.average(prediction_errors[1])
    test_error_metrics[7] = np.std(prediction_errors[1],ddof=1)
    test_error_metrics[8] = np.average(prediction_errors[2])
    test_error_metrics[9] = np.std(prediction_errors[2],ddof=1)

    """
    for k in range(len(prediction_errors[0])):
        print(prediction_errors[0][k]**2)
    print(test_set_size,len(prediction_errors[0]))
    """
    columns = (\
