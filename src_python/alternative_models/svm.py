...
if __name__ == "__main__":
    ...
    spline_degree=[int(n) for n in args.spline_degrees.split(',')]
    ...
    model_list = generate_models(spline_degree,[args.kernel_name])
    print(model_list)
	...
	svm_model = SVR(kernel=model_parameters[1],degree=model_parameters[0],C=1.0)#, epsilon=0.2)
	    ...
	    svm_model.fit(np.log(training_inputs),np.log(training_data))
        elif (args.predictor_transform==0 and args.response_transform==1):
	    svm_model.fit(training_inputs,np.log(training_data))
        elif (args.predictor_transform==1 and args.response_transform==0):
	    svm_model.fit(np.log(training_inputs),training_data)
        elif (args.predictor_transform==0 and args.response_transform==0):
	    svm_model.fit(training_inputs,training_data)

        joblib.dump(svm_model, "SVM.joblib") 
	model_size = os.path.getsize('SVR.joblib')
        print("SVM model size: %f bytes"%(model_size))
	...
	for k in range(validation_set_size):
	    input_tuple = test_inputs[k,:]*1. if args.predictor_transform==0 else np.log(test_inputs[k,:]*1.) # Note: without this cast from into to float, interpolation produces zeros
            ...
            model_val = svm_model.predict([input_tuple])[0]
