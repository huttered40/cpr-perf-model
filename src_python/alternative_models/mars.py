...
def generate_models(_spline_degrees):
    model_list = []
    for i in _spline_degrees:
        model_list.append([i])
    return model_list
...
if __name__ == "__main__":
    ...
    spline_degrees=[int(n) for n in args.spline_degrees.split(',')]
    ...
    # Generate list of model types parameterized on hyper-parameters
    model_list = generate_models(spline_degrees)
	...
	mars_model = pe.Earth(max_degree=model_parameters[0],allow_linear=False)#,smooth=True))
        model_predictions = []
        start_time_solve = time.time()
        if (args.predictor_transform==1 and args.response_transform==1):
	    mars_model.fit(np.log(training_inputs),np.log(training_data))
        elif (args.predictor_transform==0 and args.response_transform==1):
	    mars_model.fit(training_inputs,np.log(training_data))
        elif (args.predictor_transform==1 and args.response_transform==0):
	    mars_model.fit(np.log(training_inputs),training_data)
        elif (args.predictor_transform==0 and args.response_transform==0):
	    mars_model.fit(training_inputs,training_data)
	...
	for k in range(validation_set_size):
	    input_tuple = test_inputs[k,:]*1. if args.predictor_transform==0 else np.log(test_inputs[k,:]*1.) # Note: without this cast from into to float, interpolation produces zeros
            ...
            model_val = mars_model.predict([input_tuple])[0]
	...
	if (validation_error_metrics[2] < opt_error_metrics[1]):
	    ...
	    current_best_model = copy.deepcopy(mars_model)
    if (validation_set_size>0):
        Mars_Model = current_best_model
    else:
    ...
    joblib.dump(Mars_Model, "MARS_Model.joblib") 
    model_size = os.path.getsize('MARS_Model.joblib')
    print("MARS model size: %f bytes"%(model_size))

    start_time = time.time()
    model_predictions = []
    for k in range(test_set_size):
	input_tuple = test_inputs[k,:]*1. if args.predictor_transform==0 else np.log(test_inputs[k,:]*1.)
	model_val = Mars_Model.predict([input_tuple])[0]
	if (args.response_transform == 1):
	    model_val = np.exp(model_val)
	model_predictions.append(model_val)
