...
def main(args):
    ...
    spline_degrees=[int(n) for n in args.max_spline_degrees.split(',')]
	...
	for k in range(validation_set_size):
	    ...
	    model_predictions.append(inverse_transform_response(args.response_transform,mars_model.predict([configuration])[0]))
    ...
    write_statistics_to_file(args.output_file,test_error_metrics,training_error_metrics,timers,[training_set_size,validation_set_size,test_set_size],model_size,[opt_model_parameters[0]],["model:max_spline_degree"])    
