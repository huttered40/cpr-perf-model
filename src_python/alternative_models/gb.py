...
if __name__ == "__main__":
	...
	gb_model = ensemble.GradientBoostingRegressor(**params)
	    ...
	    gb_model.fit(np.log(training_inputs),np.log(training_data))
        elif (args.predictor_transform==0 and args.response_transform==1):
	    gb_model.fit(training_inputs,np.log(training_data))
        elif (args.predictor_transform==1 and args.response_transform==0):
	    gb_model.fit(np.log(training_inputs),training_data)
        elif (args.predictor_transform==0 and args.response_transform==0):
	    gb_model.fit(training_inputs,training_data)

        joblib.dump(gb_model, "GB_trees.joblib") 
	model_size = os.path.getsize('GB_trees.joblib')
        print("GB model size: %f bytes"%(model_size))
	...
	for k in range(validation_set_size):
	    input_tuple = test_inputs[k,:]*1. if args.predictor_transform==0 else np.log(test_inputs[k,:]*1.) # Note: without this cast from into to float, interpolation produces zeros
            ...
            model_val = gb_model.predict([input_tuple])[0]
