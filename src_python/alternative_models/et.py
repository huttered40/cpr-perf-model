...
def generate_models(_num_trees,_tree_depth):
    model_list = []
    for i in _num_trees:
        for j in _tree_depth:
...
if __name__ == "__main__":
    ...
    current_best_model = []
    # arithmetic sum of log Q, arithmetic sum of log^2 Q, geometric mean of relative errors, MAPE, SMAPE, RMSE
    opt_error_metrics = [100000.]*6
    for model_parameters in model_list:
	et_model = ExtraTreesRegressor(n_estimators=model_parameters[0], max_depth=model_parameters[1],random_state=0)
        model_predictions = []
        start_time_solve = time.time()
        if (args.predictor_transform==1 and args.response_transform==1):
	    et_model.fit(np.log(training_inputs),np.log(training_data))
        elif (args.predictor_transform==0 and args.response_transform==1):
	    et_model.fit(training_inputs,np.log(training_data))
        elif (args.predictor_transform==1 and args.response_transform==0):
	    et_model.fit(np.log(training_inputs),training_data)
        elif (args.predictor_transform==0 and args.response_transform==0):
	    et_model.fit(training_inputs,training_data)

        joblib.dump(et_model, "ET_Model.joblib") 
	model_size = os.path.getsize('ET_Model.joblib')
        print("ET model size: %f bytes"%(model_size))
	...
	for k in range(validation_set_size):
	    input_tuple = test_inputs[k,:]*1. if args.predictor_transform==0 else np.log(test_inputs[k,:]*1.)
            model_val = et_model.predict([input_tuple])[0]
	...
	if (validation_error_metrics[2] < opt_error_metrics[1]):
	    ...
	    current_best_model = copy.deepcopy(et_model)
    if (validation_set_size > 0):
        ET_Model = current_best_model
    else:
        ET_Model = et_model
