...
def main(args):
	...
	et_model.fit(training_configurations,training_data)
	...
	for k in range(validation_set_size):
	    configuration = transform_predictor(predictor_transform,validation_configurations[k,:]*1.)
            model_predictions.append(inverse_transform_response(args.response_transform,et_model.predict([configuration])[0]))
    ...
    model_size = get_model_size(ET_Model,"ET_Model.joblib")
