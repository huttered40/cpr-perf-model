...
if __name__ == "__main__":
	...
	nn_model = MLPRegressor(hidden_layer_sizes=hidden_layer_sizes,activation=args.activation, solver=args.solver, random_state=0)
	    ...
	    nn_model.fit(np.log(training_inputs),np.log(training_data))
        elif (args.predictor_transform==0 and args.response_transform==1):
	    nn_model.fit(training_inputs,np.log(training_data))
        elif (args.predictor_transform==1 and args.response_transform==0):
	    nn_model.fit(np.log(training_inputs),training_data)
        elif (args.predictor_transform==0 and args.response_transform==0):
	    nn_model.fit(training_inputs,training_data)

        joblib.dump(nn_model, "NN.joblib") 
	model_size = os.path.getsize('NN.joblib')
        print("NN model size: %f bytes"%(model_size))
	...
	for k in range(validation_set_size):
	    input_tuple = test_inputs[k,:]*1. if args.predictor_transform==0 else np.log(test_inputs[k,:]*1.) # Note: without this cast from into to float, interpolation produces zeros
            ...
            model_val = nn_model.predict([input_tuple])[0]
