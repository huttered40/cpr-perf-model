...
if __name__ == "__main__":
    ...
    if (args.kernel_type%10 == 0):
        ...
        mode_range_min = [32,32]
        mode_range_max = [262144,262144]
        restrict = 4194304
    else:
        assert(0)
    sampling_distribution = [args.sample_type,args.sample_type]
	...
	while (is_legal == False):
	    ...
	    input_mode2 = generate_sample(mode_range_min[1],mode_range_max[1],sampling_distribution[1],1,2)
            if ((input_mode1*input_mode2 + input_mode2*input_mode2 + input_mode1*input_mode2) > restrict):
		is_legal = False
            else:
		is_legal = True
	...
	input_tuple = (input_mode1,input_mode2,input_mode3)
        write_file[input_mode3_idx].write("%s %d %d %d %d %d %d\n"%(kernel_name,args.kernel_type,args.sample_type,input_tuple[0],input_tuple[1],input_tuple[2],i))
