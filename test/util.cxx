...
double get_wall_time(){
  ...
  return (double)time.tv_sec + (double)time.tv_usec * .000000001;
...
auto get_wall_time(){
  struct timespec time;
/* Below: For C23 and beyond
  timespec_getres(&time,TIME_UTC);
  std::cout << "What is this - " << time.tv_nsec << '\n';
*/
  timespec_get(&time,TIME_UTC);
  return time;
}
double get_wall_time_diff(const timespec& t1, const timespec& t2){
  // NOTE: this function is written to avoid cancellation and harmful shifts in floating-point values
  std::cout << "What is this - " << t1.tv_sec << ' ' << t1.tv_nsec << ' ' << t2.tv_sec << ' ' << t2.tv_nsec << '\n';
  if (t1.tv_sec == t2.tv_sec) return (t2.tv_nsec-t1.tv_nsec)*1e-9;
  else{
    double total_time = static_cast<double>(t2.tv_nsec)*1e-9;
    total_time += static_cast<double>(1000000000-t1.tv_nsec)*1e-9;
    total_time += static_cast<double>(t2.tv_sec-t1.tv_sec-1);
    return total_time;
  }
}
...
void set_cpr_param_pack(size_t nparam, performance_model::cpr_hyperparameter_pack& arg_pack, std::vector<std::string>&& hyperparameter_options, bool verbose){
  ...
  if (env_var_ptr != NULL){
    ...
    if (std::string(env_var_ptr) == "GEOMETRIC"){
      for (size_t i=0; i<nparam; i++) arg_pack.partition_spacing[i] = performance_model::parameter_range_partition::GEOMETRIC;
    }
    else if (std::string(env_var_ptr) == "UNIFORM"){
      for (size_t i=0; i<nparam; i++) arg_pack.partition_spacing[i] = performance_model::parameter_range_partition::UNIFORM;
    }
    else if (std::string(env_var_ptr) == "SINGLE"){
      for (size_t i=0; i<nparam; i++) arg_pack.partition_spacing[i] = performance_model::parameter_range_partition::SINGLE;
    }
    else if (std::string(env_var_ptr) == "AUTOMATIC"){
      for (size_t i=0; i<nparam; i++) arg_pack.partition_spacing[i] = performance_model::parameter_range_partition::AUTOMATIC;
    }
  ...
  if (verbose){
    for (size_t i=0; i<nparam; i++) std::cout << arg_pack.partition_info[i] << " ";
    std::cout << '\n';
  }
...
void set_cprg_param_pack(size_t nparam, performance_model::cprg_hyperparameter_pack& arg_pack, bool verbose){
...
void evaluate(size_t nparam,
  ...
  for (size_t i=0; i<size; i++){
    auto start_inference_latency = get_wall_time();
    ...
    double inference_latency = get_wall_time_diff(start_inference_latency,get_wall_time());
