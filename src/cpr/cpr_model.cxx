...
double multilinear_product(const double* multi_vector, const size_t* cells, const int* mode_lengths, size_t order, size_t rank){
...
double multilinear_product_packed(const double* multi_vector, size_t nelements, size_t order, size_t rank){
  double t_val = 0;
...
struct MLogQ2{
    ...
    static void Get_RHS(CTF::World* dw, CTF::Tensor<>* T, CTF::Tensor<>* O, std::vector<CTF::Tensor<>*>& A, size_t num, double reg, double mu, CTF::Tensor<>* grad, CTF::Tensor<>* Hessian){
        CTF::Tensor<> M(O);
        std::vector<int> mode_list(A.size());
        for (size_t j=0; j<A.size(); j++) mode_list[j]=j;
        ...
        for (size_t i=0; i<A.size(); i++){
            ...
            while (mu >= barrier_stop){
                ...
                while (t<max_newton_iter){
                    ...
                    for (int64_t j=0; j<num_nnz_elems; j++){
...
struct MSE{
    static void Get_RHS(CTF::World* dw, CTF::Tensor<>* T, std::vector<CTF::Tensor<>*>& A, size_t num, double reg, CTF::Tensor<>* grad){
  ...
  delete lst_mat[num];
    ...
    static void step(CTF::World* dw, CTF::Tensor<>* T, CTF::Tensor<>* O, std::vector<CTF::Tensor<>*>& A, double reg, uint64_t nnz){
        std::vector<int> fm_mode_types(2,NS);
        std::vector<int> fm_mode_lengths(2,0);
        ...
        for (size_t i=0; i<A.size(); i++){
            ...
            fm_mode_lengths[0]=A[i]->lens[0]; fm_mode_lengths[1]=A[i]->lens[1];
            CTF::Tensor<> g(2,&fm_mode_lengths[0],&fm_mode_types[0],*dw);
            MSE::Get_RHS(dw,T,A,i,reg*nnz,&g);
...
double cpd_als(CTF::World* dw, CTF::Tensor<>* T_in, CTF::Tensor<>* O, std::vector<CTF::Tensor<>*> X, double reg, double model_convergence_tolerance, size_t max_nsweeps, loss_function t_loss = loss_function::MSE){
    ...
    uint64_t nnz = O->nnz_loc;
    double err=100000000.;
    std::cout << "nnz - " << nnz << std::endl;
    for (size_t i=0; i<max_nsweeps; i++){
      if (i>0){
        ...
        for (size_t j=0; j<X.size(); j++) mode_list[j]=j;
...
/*
    ...
    double err=err_prev;
*/
    size_t n_newton_iterations=0;
    auto TT = *T_in;
    sparse_log(&TT);
    for (size_t i=0; i<max_nsweeps; i++){
      if (i>0){
        size_t _n_newton_iterations = MLogQ2::step(dw,T_in,O,X,reg,barrier_start,barrier_stop,barrier_reduction_factor,factor_matrix_convergence_tolerance, max_newton_iter);
        normalize(X);
...
void partition_space(size_t max_num_obs_per_cell, size_t start_idx, size_t end_idx, std::vector<double>& features, std::vector<double>& nodes, double max_spacing_factor){
  ...
  size_t midpoint_idx = std::lower_bound(features.begin()+start_idx,features.begin()+end_idx+1,midpoint) - features.begin();
...
size_t get_interval_index(double val, size_t num_nodes, const double* _nodes, parameter_range_partition node_spacing){
...
*/
  ...
  size_t start=0;
  assert(num_nodes>=2);
  size_t end=std::max(num_nodes-2,start);//NOTE: This works for parameter_range_partition::SINGLE spacing too, because we already checked for the last node
...
size_t get_node_index(double val, size_t num_nodes, const double* _nodes, parameter_range_partition node_spacing_type){
  ...
  size_t start=0;
  assert(num_nodes>=2);
  size_t end=std::max(num_nodes-2,start);
  while (start < end){
    size_t mid = start + (end-start)/2;
...
std::vector<double> generate_nodes(double _min, double _max, size_t num_grid_pts, parameter_range_partition node_spacing_type, parameter_type param_type){
...
void cpr_model::init(
  std::vector<int>& cells_info, size_t num_configurations, const double* features){
    ...
    for (size_t i=0; i<this->order; i++){
        if (_hyperparameters->partition_spacing[i]==parameter_range_partition::SINGLE){
          ...
          if (i<(this->order-1)) _parameters->knot_index_offsets[i+1]=temp_nodes.size()+_parameters->knot_index_offsets[i];
            ...
            size_t max_num_distinct_obs_per_cell = cells_info[i];
            std::vector<double> temp_nodes;
            std::vector<double> features_copy(num_configurations);
            for (size_t j=0; j<num_configurations; j++){
              features_copy[j] = std::max(features[this->order*j+i],this->param_range_min[i]);
            ...
            //assert(temp_nodes.size()>1);
            for (size_t j=0; j<temp_nodes.size(); j++){ if (j>0) { assert(temp_nodes[j]>temp_nodes[j-1]); } _parameters->knot_positions.push_back(temp_nodes[j]); }
            assert(temp_nodes.size()>0);
            _parameters->num_partitions_per_dimension[i] = temp_nodes.size()-1;
    ...
    for (size_t i=0; i<this->order; i++) std::cout << _parameters->num_partitions_per_dimension[i] << ' ';
    std::cout << std::endl;
...
cpr_model::cpr_model(int nparam, const parameter_type* parameter_types, const hyperparameter_pack* pack) : model(nparam,parameter_types,pack){
  ...
  for (size_t i=0; i<_hyperparameters->nparam; i++) assert(_hyperparameters->partition_spacing[i] != parameter_range_partition::CUSTOM);
...
double cpr_model::predict(const double* configuration) const{
  ...
  auto _parameters = dynamic_cast<cpr_parameter_pack*>(this->parameters);
      ...
      const size_t& rank = _parameters->cp_rank;
      assert(fm!=nullptr);
      assert(rank>=1);
      ...
      for (size_t j=0; j<this->order; j++){
          ...
          if (configuration[j] < _parameters->knot_positions[_parameters->knot_index_offsets[j]]){
              ...
              int fm_offset = 0;
              for (size_t k=0; k<j; k++) fm_offset += _parameters->num_partitions_per_dimension[k]*rank;
      ...
      for (size_t j=0; j<modes_to_interpolate.size(); j++){
        ...
        size_t num_interp_pts = (1<<modes_to_interpolate.size());
        for (size_t j=0; j<num_interp_pts; j++){
          size_t interp_id = j;
          ...
          double coeff = 1;
          for (size_t l=0; l<modes_to_interpolate.size(); l++){
...
bool cpr_model::train(size_t& num_configurations, const double*& configurations, const double*& runtimes, model_fit_info* fit_info){
  auto _hyperparameters = dynamic_cast<cpr_hyperparameter_pack*>(this->hyperparameters);
  auto _parameters = dynamic_cast<cpr_parameter_pack*>(this->parameters);
  auto _fit_info = dynamic_cast<cpr_model_fit_info*>(fit_info);
  bool compute_fit_error = _fit_info!=nullptr;
      if (!model::train(num_configurations,configurations,runtimes,fit_info)) return false;
      ...
      for (size_t i=0; i<_parameters->num_partitions_per_dimension.size(); i++){
      ...
      std::map<std::vector<size_t>,double> node_data_dict;
      std::map<std::vector<size_t>,int> node_count_dict;
      ...
      for (size_t i=0; i<num_configurations; i++){
          ...
          for (size_t j=0; j<this->order; j++){
            ...
            assert(element_key[j] >= 0 && element_key[j] < static_cast<size_t>(_parameters->num_partitions_per_dimension[j]));
          }
      ...
      for (size_t i=0; i<this->order; i++){
        for (size_t j=0; j<this->Projected_Omegas[i].size(); j++){
      ...
      std::vector<int> fm_mode_types(2,NS);
      for (size_t i=0; i<this->order; i++){
          std::vector<int> fm_mode_lengths = {static_cast<int>(_hyperparameters->cp_rank),static_cast<int>(_parameters->num_partitions_per_dimension[i])};
       ...
       size_t num_re_inits = 0;
      if (_hyperparameters->loss == loss_function::MSE){
        while (num_re_inits < _hyperparameters->max_num_re_inits){
         for (size_t i=0; i<this->order; i++) init_factor_matrix(FM1[i],_hyperparameters->loss);
      ...
      size_t total_elements_across_all_factor_matrices = 0;
      for (size_t i=0; i<this->order; i++){
      ...
      for (size_t i=0; i<this->order; i++){
        ...
        assert(static_cast<int64_t>(_parameters->num_partitions_per_dimension[i]*_hyperparameters->cp_rank) == num_nnz_elems);
      ...
      if (compute_fit_error){
        ...
        if (do_i_want_to_print_out){
          ...
          for (size_t i=0; i<_parameters->knot_positions.size(); i++){
...
//          assert(std::abs(tensor_elem_prediction2-tensor_elem_prediction)<=1e-6); 
          if (do_i_want_to_print_out && quad_err <= .1 && low_rank_err1 <= .1 && rel_err >= .5){
            ...
            std::vector<size_t> element_key_left = {element_key[0],std::max(0ul,element_key[1]-1)};
            std::vector<size_t> element_key_right = {element_key[0],std::min(static_cast<size_t>(_parameters->num_partitions_per_dimension[1]),element_key[1]+1)};
...
double cprg_model::predict(const double* configuration) const{
  ...
  auto _parameters = dynamic_cast<cprg_parameter_pack*>(this->parameters);
      ...
      std::vector<std::pair<size_t,size_t>> element_index_modes_list;
      // Because each tensor element is associated with the mid-point of the corresponding grid-cell (as a tensor product of mid-points of each parameter's range), we also want to get the surrounding nodes
      // calculation of the nodes to use for a (multi)linear interpolation strategy is nontrivial
      for (size_t j=0; j<modes_to_interpolate.size(); j++){
        // NOTE: This loop iteration exists only if there always exists a midpoint to left and right!
        ...
        for (size_t j=0; j<num_interp_pts; j++){
          ...
          for (size_t l=0; l<modes_to_interpolate.size(); l++){
            if (interp_id_list[l] == 0){
              size_t order_id = modes_to_interpolate[l];
              size_t& temp_id = element_index_modes_list[l].first;
              assert(temp_id>=0);
              double left_point = _hyperparameters->partition_spacing[order_id]==parameter_range_partition::SINGLE ? _parameters->knot_positions[_parameters->knot_index_offsets[order_id]+temp_id] : get_midpoint_of_two_nodes(temp_id, _parameters->num_partitions_per_dimension[order_id]+1,&_parameters->knot_positions[_parameters->knot_index_offsets[order_id]], _hyperparameters->partition_spacing[order_id]);
              if (_hyperparameters->partition_spacing[order_id]!=parameter_range_partition::SINGLE) assert(!(temp_id+2==static_cast<size_t>(_parameters->num_partitions_per_dimension[order_id]+1)));
              double right_point = _hyperparameters->partition_spacing[order_id]==parameter_range_partition::SINGLE ? _parameters->knot_positions[_parameters->knot_index_offsets[order_id]+temp_id+1] : get_midpoint_of_two_nodes(temp_id+1, _parameters->num_partitions_per_dimension[order_id]+1,&_parameters->knot_positions[_parameters->knot_index_offsets[order_id]], _hyperparameters->partition_spacing[order_id]);
              if (do_i_want_to_print_out) std::cout << "Points(0): " << left_point << ' ' << right_point << std::endl;
              coeff *= std::max(0.,(1-(std::abs(configuration[order_id]-left_point))/(right_point-left_point)));
              assert(coeff >= 0);
            ...
            if (interp_id_list[l] == 1){
              size_t order_id = modes_to_interpolate[l];
              size_t& temp_id = element_index_modes_list[l].second;
              assert(temp_id>=1);
              //if (_hyperparameters->partition_spacing[order_id]==parameter_range_partition::SINGLE) assert(!(temp_id==_parameters->num_partitions_per_dimension[order_id]+1));
              double left_point = _hyperparameters->partition_spacing[order_id]==parameter_range_partition::SINGLE ? _parameters->knot_positions[_parameters->knot_index_offsets[order_id]+temp_id-1] : get_midpoint_of_two_nodes(temp_id-1, _parameters->num_partitions_per_dimension[order_id]+1,&_parameters->knot_positions[_parameters->knot_index_offsets[order_id]], _hyperparameters->partition_spacing[order_id]);
              //if (_hyperparameters->partition_spacing[order_id]==parameter_range_partition::SINGLE) assert(!(temp_id+1==_parameters->num_partitions_per_dimension[order_id]+1));
...
//          factor_row_list.reserve(rank*this->order);
          ...
          for (size_t l=0; l<this->order; l++){
            if (local_interp_map[l]==1){
              size_t& temp_id = interp_id_list[interp_counter]==0 ? element_index_modes_list[interp_counter].first : element_index_modes_list[interp_counter].second;
              for (size_t ll=0; ll<rank; ll++){
                factor_row_list.push_back(fm[factor_matrix_offset + temp_id*rank+ll]);
              }
              interp_counter++;
              ...
              if (decisions[l]==0){
                ...
                for (size_t ll=0; ll<rank; ll++){
                  ...
                  for (size_t kk=0; kk<1+_parameters->spline_degree; kk++){
...
bool cprg_model::train(size_t& num_configurations, const double*& configurations, const double*& runtimes, model_fit_info* fit_info){
      if (!cpr_model::train(num_configurations,configurations,runtimes,fit_info)) return false;
      auto _hyperparameters = dynamic_cast<cprg_hyperparameter_pack*>(this->hyperparameters);
      auto _parameters = dynamic_cast<cprg_parameter_pack*>(this->parameters);
      assert(_hyperparameters != nullptr);
      ...
      for (size_t i=0; i<this->numerical_modes.size(); i++){
      ...
      for (size_t i=0; i<this->order; i++){
        ...
        size_t local_projected_set_size = 0;
        int max_elem = *std::max_element(this->Projected_Omegas[i].begin(),this->Projected_Omegas[i].end());
        for (int j=0; j<_parameters->num_partitions_per_dimension[i]; j++){
          assert(static_cast<size_t>(j)<this->Projected_Omegas[i].size());
        ...
        if (_parameters->cp_rank > local_projected_set_size ||
          ...
          size_t jump = 1+_parameters->spline_degree+1+_parameters->cp_rank;
          for (size_t k=0; k<jump; k++){
        ...
        assert(static_cast<int>(local_projected_set_size) == _parameters->num_partitions_per_dimension[i]);
        ...
        for (int j=_parameters->num_partitions_per_dimension[i]-local_projected_set_size; j<_parameters->num_partitions_per_dimension[i]; j++){
          ...
          if (this->Projected_Omegas[i][j] >= min(projection_set_size_threshold_[i],max_elem)){
            ...
            for (size_t k=0; k<_parameters->cp_rank; k++){
      ...
      size_t num_elements_to_keep = local_projected_set_size;// should be <= local_projected_set_size
      //std::cout << "num_elements_to_keep - " << num_elements_to_keep << std::endl;
      for (size_t j=0; j<local_projected_set_size; j++){
         if (left_singular_matrix[j]<0) left_singular_matrix[j] *= (-1);
       }
       for (size_t j=0; j<_parameters->cp_rank; j++){
...
*/
       ...
       for (size_t j=1; j<=_parameters->spline_degree; j++){
         for (int k=_parameters->num_partitions_per_dimension[i]-num_elements_to_keep; k<_parameters->num_partitions_per_dimension[i]; k++){
           ...
           assert(static_cast<size_t>(k)<valid_tensor_cell_points[i].size());
       ...
       for (size_t k=0; k<num_elements_to_keep; k++){
